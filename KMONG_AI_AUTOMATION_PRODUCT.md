...
> 구현 반영: 필수 `client_id`, Telegram 토큰/채팅 ID·웹훅 URL·env 헤더의
> 고객 전용 이름 우선/공용 폴백, 다른 고객 namespace 참조 차단, validate 이름 진단.
> 공용 폴백과 동일 프로세스의 제한은 `docs/botkit.md`에 명시했다.
> [검증·릴리스 기록](docs/releases/p0-2.md) 참고. P0-3는 아직 미구현이다.
